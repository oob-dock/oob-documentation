...
LANGUAGE SQL
AS $$
select  SUM(CASE WHEN tb.sha_business_document_number IS NULL THEN 1 ELSE 0 END)::int AS Clientes_Unicos_PF_Total,
		SUM(CASE WHEN tb.sha_business_document_number IS NOT NULL THEN 1 ELSE 0 END)::int AS Clientes_Unicos_PJ_Total
from (
