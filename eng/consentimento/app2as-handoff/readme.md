...
| `tpp.name`                          | TPP name for display on the return screen.                                                                                                                                                                                           |
| `tpp.logoUrl`                       | URL with the TPP logo for display on the return screen                                                                                                 |
| `errorCommand.type`                 | Error type. Same `enum` as APP2AS: `CPF_MISMATCH`, `CNPJ_MISMATCH`, `EXPIRED_CONSENT`, `RESOURCE_MUST_CONTAIN_ID`, `GENERIC_ERROR`, `OIDC_ERROR`, `DISCOVERY_ERROR`, `RESOURCE_MUST_CONTAIN_ID_SELECTABLE_PRODUCTS`, `DISCOVERY_TIMEOUT`, `INVALID_STATUS_CONFIRMATION` |
