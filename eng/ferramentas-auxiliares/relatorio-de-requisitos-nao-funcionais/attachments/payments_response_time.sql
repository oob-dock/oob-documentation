create or replace function public.payments_response_time(dt_end date)
 returns table(organizacao uuid, data_metrica date, periodo text, metodo_http text, endpoint text, less_than_half_second int8, more_than_half_second int8)
 language plpgsql
as $function$
	...
	select (dt_end + INTERVAL '1 days')::varchar INTO dt_end_interval;
   	...
   	return query
		...
		select 
			...
			case
				when date_part('isodow', e.measure_date) = 1 then 'Moday'
				when date_part('isodow', e.measure_date) = 2 then 'Tuesday'
				when date_part('isodow', e.measure_date) = 3 then 'Wednesday'
				when date_part('isodow', e.measure_date) = 4 then 'Thursday'
				when date_part('isodow', e.measure_date) = 5 then 'Friday'
				when date_part('isodow', e.measure_date) = 6 then 'Saturday'
				when date_part('isodow', e.measure_date) = 7 then 'Sunday'
			end periodo,
			e.httpMethod as metodo_http,
			e.endpoint,
			count (*) filter (where e.processTimespan <= 1500) as less_than_half_second,
			count (*) filter (where e.processTimespan > 1500) as more_than_half_second
		from endpoints e
